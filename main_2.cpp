...
unordered_map <string, bitset < 10010 > > from(10000);
unordered_map <string, bitset < 10010 > > to(10000);
unordered_map <string, bitset < 10010 > > keyword(400000);
...
void add(string &route){
	...
	length_of_mail[total_mail_length].set(ID);
...
void longest(){	
    for(auto i = length_of_mail.rbegin(); i != length_of_mail.rend(); i++){
        ...
        if(!((i -> second & exist_mail_id).none())){
            bitset<10010> test = i -> second & exist_mail_id;
            cout <<  test._Find_first() <<  " " << i -> first << "\n";
            return ; 
        }
    }
	cout << "-" << "\n";
